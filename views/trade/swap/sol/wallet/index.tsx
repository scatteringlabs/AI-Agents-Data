...
import { initializeUmi, searchAssets } from "@/utils/umi";
import { publicKey } from "@metaplex-foundation/umi";
import { DasApiAssetList } from "@metaplex-foundation/digital-asset-standard-api";
import MyNFTCard from "../MPL404/MyNFTCard";
...
const WalletPage = ({
  ...
  // const { publicKey: userPubliccKey, connected } = useWallet();
  const { wallets: solanaWallets } = useSolanaWallets();
  const address = useMemo(() => solanaWallets?.[0]?.address, [solanaWallets]);
  const userPubliccKey = useMemo(
    () => (address ? new PublicKey(address) : ""),
    [address],
  );
  const router = useRouter();
  const [loading, setLoading] = useState<boolean>(true);
  const [myAssetsList, setMyAssetsList] = useState<DasApiAssetList | null>(
    null,
  );
  const [tokenBalance, setTokenBalance] = useState(0);
  const testAddress = useMemo(
    () => userPubliccKey?.toString() || "",
    [userPubliccKey],
  );
  // const [testAddress, setTestAddress] = useState(
  //   "8DTvkcCJRRYJm6bfpctioo7YuBhtu6XNpD4tkbj3fy9P",
  // );
  console.log("userPubliccKey", userPubliccKey);
  ...
  }, [connection, collectionDetails, testAddress, userPubliccKey]);
  const cardLoading = useMemo(
    () => walletLoading || loading,
    [walletLoading, loading],
  );
  const usdtFloorPrice = useMemo(
    () => Number(solPrice) * Number(collectionStats?.floorPrice),
    [solPrice, collectionStats],
  );
  const tokenValue = useMemo(
    () =>
      `${(Number(collectionDetails?.price_in_usd) * Number(tokenBalance))?.toFixed(2)}`,
    [collectionDetails, tokenBalance],
  );
  const NFTsValue = useMemo(
    () =>
      `${((Number(myAssetsList?.total) || 0) * usdtFloorPrice)?.toFixed(2)}`,
    [myAssetsList, usdtFloorPrice],
  );
  const totalValue = useMemo(
    () => Number(tokenValue) + Number(NFTsValue),
    [NFTsValue, tokenValue],
  );
  const cardData: CardData[] = useMemo(
    () => [
      ...
      },
      {
        ...
        title: "Token Amount",
        value: `${tokenBalance.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
        hasButton: true,
      },
      {
        icon: (
      ...
      },
      {
        ...
        title: "NFTs value",
        value: `$${Number(NFTsValue).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      },
    ],
    [myAssetsList, tokenBalance, NFTsValue, tokenValue, totalValue],
  );
  if (!userPubliccKey) {
    return <NeedConnectCard type="sol" />;
  }
  return (
    <Box>
      <AssetCard
        ...
        setActiveTab={() => {
          router.push(
            `/collection/${ChainIdByName?.[Number(collectionDetails?.chain_id)]}/${collectionDetails?.erc20_address}`,
            undefined,
            {
              shallow: true,
            },
      ...
      {!myAssetsList?.total && !cardLoading ? (
